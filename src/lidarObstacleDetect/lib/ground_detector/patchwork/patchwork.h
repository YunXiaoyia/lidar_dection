...
typedef std::vector<Ring> Zone;
    ...
    private:
        ...
        bool use_ground_temporal_stabilizer_;
        double ground_plane_ema_alpha_;
        int ground_cache_max_miss_;
        double ground_enter_dist_;
        double nonground_exit_dist_;
        double small_patch_fallback_z_;

        double min_range_z1_; 
        ...
        double min_range_z1_sq_ ;
        double min_range_z2_sq_ ;
        double min_range_z3_sq_ ;
        double min_range_z4_sq_ ;
        ...
        // 用于动态体素滤波的叶子大小
        std::vector<double> leaf_sizes_;

        std::vector<Zone> ConcentricZoneModel_;
        std::vector<std::vector<std::vector<GroundPlaneState>>> plane_cache_;
        ...
        bool fit_patch_ground(const int zone_idx,
                              const pcl::PointCloud<pcl::PointXYZI> &src,
                              PatchGroundResult &result);
        bool is_reliable_plane(const PatchGroundResult& result) const;
        GroundPlaneState& get_plane_state(int zone_idx, int ring_idx, int sector_idx);
        const GroundPlaneState* get_plane_state(int zone_idx, int ring_idx, int sector_idx) const;
        void update_plane_state(GroundPlaneState& state, const PatchGroundResult& result);
        void classify_with_plane(const pcl::PointCloud<pcl::PointXYZI>& src,
                                 const Eigen::Vector3f& normal,
                                 float d,
                                 pcl::PointCloud<pcl::PointXYZI>& ground,
                                 pcl::PointCloud<pcl::PointXYZI>& nonground) const;
        void classify_small_patch(const pcl::PointCloud<pcl::PointXYZI>& src,
