...
class ExtendedKalmanCA : public IKalmanFilter {
public:
    ExtendedKalmanCA() = default;
    virtual ~ExtendedKalmanCA() = default;
    ...
    void Init(const Eigen::VectorXf& position, const Eigen::VectorXf& velocity) {
        ...
        meas_covariance(0, 0) = 2; // 
        meas_covariance(1, 1) = 2;
        meas_covariance(2, 2) = 10; // 5
        meas_covariance(3, 3) = 10; // 5
    ...
    Eigen::VectorXf update(const Eigen::VectorXf& position, const Eigen::VectorXf& velocity) {
        ...
        // PositionMeasurement 只包含位置，不包含速度
        // pos_meas.vx() = velocity(0);  // 不存在
        // pos_meas.vy() = velocity(1);  // 不存在
...
PERCEPTION_REGISTER_KALMAN_FILTER(ExtendedKalmanCA);
