...
/// --------------------------------------------------------------------------
void AssignmentProblemSolver::assignmentsuboptimal2(int* assignment, double* cost, double* distMatrixIn, int nOfRows,
                                                    int nOfColumns) {
    int n = 0, row = 0, col = 0, tmpRow = 0, tmpCol = 0, nOfElements = 0;
...
// assignments.
/// --------------------------------------------------------------------------
void AssignmentProblemSolver::assignmentsuboptimal1(int* assignment, double* cost, double* distMatrixIn, int nOfRows,
                                                    int nOfColumns) {
    ...
    *cost = 0;
    ...
    if (infiniteValueFound) {
        ...
        while (repeatSteps) {
            ...
            /* step 2: reject assignments of multiply validated observations to
             * singly validated tracks */
            if (nOfColumns > 1) {
                for (row = 0; row < nOfRows; row++) {
                    singleValidationFound = false;
                    for (col = 0; col < nOfColumns; col++) {
        ...
        /* for each multiply validated observation that validates only with
         * singly validated  */
        ...
        for (col = 0; col < nOfColumns; col++) {
            if (nOfValidTracks[col] > 1) {
                ...
                if (allSinglyValidated) {
                    ...
                    *cost += minValue;
                    for (n = 0; n < nOfRows; n++) distMatrix[n + nOfRows * col] = DBL_MAX;
                    for (n = 0; n < nOfColumns; n++) distMatrix[tmpRow + nOfRows * n] = DBL_MAX;
                }
            }
        }
    } /* if(infiniteValueFound) */
    ...
    while (true) {
        ...
        if (minValue != DBL_MAX) {
            assignment[tmpRow] = tmpCol;
            *cost += minValue;
            for (n = 0; n < nOfRows; n++) distMatrix[n + nOfRows * tmpCol] = DBL_MAX;
            for (n = 0; n < nOfColumns; n++) distMatrix[tmpRow + nOfRows * n] = DBL_MAX;
        } else
            break;
