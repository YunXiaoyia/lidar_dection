...
class GatedHungarianMatcher {
 ...
 private:
  ...
  void MatchInit(const SecureMat<T>& cost_mat);
  ...
  void OptimizeConnectedComponent(const SecureMat<T>& cost_mat,
   ...
   * @return: nothing */
  void UpdateGatingLocalCostsMat(
    const SecureMat<T>& cost_mat,
    const std::vector<size_t>& row_component,
                                 const std::vector<size_t>& col_component);
...
void GatedHungarianMatcher<T>::Match(
    ...
    std::vector<size_t>* unassigned_rows,
    std::vector<size_t>* unassigned_cols) {
  Match(cost_thresh, cost_thresh, opt_flag, cost_mat, assignments, unassigned_rows,
        unassigned_cols);
}
...
void GatedHungarianMatcher<T>::Match(
  ...
  this->ComputeConnectedComponents(cost_mat, &row_components, &col_components);

  /* compute assignments */
  assignments_ptr_->clear();
  assignments_ptr_->reserve(std::max(rows_num_, cols_num_));
  for (size_t i = 0; i < row_components.size(); ++i) {
    this->OptimizeConnectedComponent(cost_mat, row_components[i], col_components[i]);
  }
...
template <typename T>
void GatedHungarianMatcher<T>::MatchInit(const SecureMat<T>& cost_mat) {
  /* get number of rows & cols */
  rows_num_ = cost_mat.height();
  cols_num_ = (rows_num_ == 0) ? 0 : cost_mat.width();
...
void GatedHungarianMatcher<T>::ComputeConnectedComponents(
  ...
  for (size_t i = 0; i < rows_num_; ++i) {
    for (size_t j = 0; j < cols_num_; ++j) {
      if (is_valid_cost_(const_mat(i, j))) {
...
void GatedHungarianMatcher<T>::OptimizeConnectedComponent(
  ...
  if (local_rows_num == 1 && local_cols_num == 1) {
    ...
    if (is_valid_cost_(const_mat(idx_r, idx_c))) {
  ...
  UpdateGatingLocalCostsMat(const_mat, row_component, col_component);
  ...
  for (size_t i = 0; i < local_assignments.size(); ++i) {
    ...
    if (!is_valid_cost_(const_mat(global_row_idx, global_col_idx))) {
      continue;
    }
...
void GatedHungarianMatcher<T>::UpdateGatingLocalCostsMat(
  ...
  for (size_t i = 0; i < row_component.size(); ++i) {
    for (size_t j = 0; j < col_component.size(); ++j) {
      const T& current_cost = const_mat(row_component[i], col_component[j]);
