...
inline double logGauss(const Vec& s, const SymMatrix& S) {
   SymMatrix Si(S.size1(), S.size2());
   Float detS;
   Float rcond = UdUinversePD(Si, detS, S);  // Si = inv(S)  detS为S的行列式
   Numerical_rcond rclimit;
   rclimit.check_PD(rcond, "S not PD");
   // log(exp(-0.5 * (s' * Si * s)) / sqrt(2pi^ns * |S|))   PDA中beta_i的分子
...
public:
   ...
   void init(const vector< size_t >& zNum, size_t tNum) {
      m_tNum = tNum; // track的数量
      m_mNum = zNum.size(); // 表示几个传感器，对应的观测目标的数量
      ...
      for (size_t m = 0; m < m_mNum; m++) {
         ...
         Omega.push_back(omega);   // 几个传感器就有几个 omega, lambda, mat...
   ...
   int getAssociations() {
      ...
      Association assoc;  // 含有两个变量结构体的vector  

      for (size_t m = 0; m < m_mNum; m++) {   // m_mNum观测的传感器数量，目前是1
         m_assocVec.clear();
         assoc.clear();
