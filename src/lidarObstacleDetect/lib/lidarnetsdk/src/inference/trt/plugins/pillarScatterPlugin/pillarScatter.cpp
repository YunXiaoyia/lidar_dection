...
#include <cstring>
#include "kernel.h"
#include "pillarScatterKernel.cuh"
...
PillarScatterPlugin::PillarScatterPlugin(size_t h, size_t w) : feature_y_size_(h), feature_x_size_(w) {}
...
nvinfer1::DimsExprs PillarScatterPlugin::getOutputDimensions(int outputIndex, const nvinfer1::DimsExprs* inputs,
                                                             int nbInputs,
...
bool PillarScatterPlugin::supportsFormatCombination(int pos, const nvinfer1::PluginTensorDesc* inOut, int nbInputs,
    ...
    try {
        ...
        if (inputType == nvinfer1::DataType::kHALF) {
            ...
            cudaMemsetAsync(spatial_feature_data, 0, batchSize * numFeatures * featureY * featureX * sizeof(half),
                            stream);
            status =
                pillarScatterKernelLaunch<half>(batchSize, maxPillarNum, numFeatures, pillar_features_data, coords_data,
                                                params_data, featureX, featureY, spatial_feature_data, stream);
...
nvinfer1::DataType PillarScatterPlugin::getOutputDataType(int index, const nvinfer1::DataType* inputTypes,
                                                          int nbInputs) const noexcept {
    return inputTypes[0];
}

const char* PillarScatterPlugin::getPluginType() const noexcept { return PLUGIN_NAME; }

const char* PillarScatterPlugin::getPluginVersion() const noexcept { return PLUGIN_VERSION; }

int PillarScatterPlugin::getNbOutputs() const noexcept { return 1; }

int PillarScatterPlugin::initialize() noexcept { return 0; }

void PillarScatterPlugin::terminate() noexcept {}

size_t PillarScatterPlugin::getSerializationSize() const noexcept { return 3 * sizeof(size_t); }
...
void PillarScatterPlugin::destroy() noexcept { delete this; }

void PillarScatterPlugin::setPluginNamespace(const char* libNamespace) noexcept { mNamespace = libNamespace; }

const char* PillarScatterPlugin::getPluginNamespace() const noexcept { return mNamespace.c_str(); }
...
const char* PillarScatterPluginCreator::getPluginName() const noexcept { return PLUGIN_NAME; }

const char* PillarScatterPluginCreator::getPluginVersion() const noexcept { return PLUGIN_VERSION; }

const PluginFieldCollection* PillarScatterPluginCreator::getFieldNames() noexcept { return &mFC; }
...
IPluginV2* PillarScatterPluginCreator::deserializePlugin(const char* name, const void* serialData,
...
void PillarScatterPluginCreator::setPluginNamespace(const char* libNamespace) noexcept { mNamespace = libNamespace; }

const char* PillarScatterPluginCreator::getPluginNamespace() const noexcept { return mNamespace.c_str(); }
