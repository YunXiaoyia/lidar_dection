...
__global__ void generateVoxels_kernel(int max_num_points, float *points, unsigned int *points_size, float min_x_range,
                                      float max_x_range, float min_y_range, float max_y_range, float min_z_range,
                                      float max_z_range, float pillar_x_size, float pillar_y_size, float pillar_z_size,
                                      int grid_y_size, int grid_x_size, int num_point_values, int max_points_per_voxel,
    ...
    if (num_point_values == 4) {
        pw = points[num_point_values * point_idx + 3];
...
__global__ void generateBaseFeatures_kernel(int batch_size, unsigned int *mask, float *voxels, int grid_y_size,
                                            int grid_x_size, unsigned int *pillar_num, int max_pillar_num,
                                            int max_points_per_voxel, int num_point_values, float *voxel_features,
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (num_point_values == 4) {
            ...
            for (int k = 0; k < 5; k++) voxel_features[5 * outIndex + k] = voxels[5 * inIndex + k];
        } else if (num_point_values == 3) {
            for (int k = 0; k < 3; k++) voxel_features[3 * outIndex + k] = voxels[3 * inIndex + k];
        }
    }
}
...
void generateVoxels_launch(int batch_size, int max_num_points, float *points, unsigned int *points_size,
    ...
    generateVoxels_kernel<<<blocks, threads, 0, stream>>>(
        max_num_points, points, points_size, min_x_range, max_x_range, min_y_range, max_y_range, min_z_range,
        max_z_range, pillar_x_size, pillar_y_size, pillar_z_size, grid_y_size, grid_x_size, num_point_values,
        max_points_per_voxel, mask, voxels);
}
...
void generateBaseFeatures_launch(int batch_size, unsigned int *mask, float *voxels, int grid_y_size, int grid_x_size,
    ...
    generateBaseFeatures_kernel<<<blocks, threads, 0, stream>>>(
        batch_size, mask, voxels, grid_y_size, grid_x_size, pillar_num, max_pillar_num, max_points_per_voxel,
        num_point_values, voxel_features, voxel_num_points, coords);
}

__global__ void generateFeatures_kernel(int batch_size, int dense_pillar_num, float *voxel_features,
                                        unsigned int *voxel_num_points, unsigned int *coords, unsigned int *params,
                                        float voxel_x, float voxel_y, float voxel_z, float range_min_x,
                                        float range_min_y, float range_min_z, unsigned int voxel_features_size,
                                        unsigned int max_points, unsigned int max_voxels, float *features) {
    ...
    for (int k = 0; k < 5; k++) {
        pillarSM[pillar_idx_inBlock][point_idx][k] =
            voxel_features[5 * (dense_pillar_idx * max_points + point_idx) + k];
    }
    __syncthreads();
    // calculate sm
    ...
    if (point_idx < pointsNumSM[pillar_idx_inBlock]) {
        ...
        if (5 + 5 < voxel_features_size) pillarOutSM[pillar_idx_inBlock][point_idx][warp_size + 5] = center.z;
    } else {
        for (int k = 0; k < voxel_features_size; k++) pillarOutSM[pillar_idx_inBlock][point_idx][k] = 0;
    }
    __syncthreads();
...
__global__ void generateFeatures_kernel_3x(int batch_size, int dense_pillar_num, float *voxel_features,
                                           unsigned int *voxel_num_points, unsigned int *coords, unsigned int *params,
                                           float voxel_x, float voxel_y, float voxel_z, float range_min_x,
                                           float range_min_y, float range_min_z, unsigned int voxel_features_size,
                                           unsigned int max_points, unsigned int max_voxels, float *features) {
    ...
    // load src
    __shared__ float pillarSM[4][64][3];      // up to 64 points per pillar
    __shared__ float4 pillarSumSM[4];         // 4*4
    __shared__ int4 cordsSM[4];               // 4*4
    __shared__ int pointsNumSM[4];            // 4
    __shared__ float pillarOutSM[4][64][11];  // up to 11 features per point
    ...
    for (int k = 0; k < 3; k++) {
        pillarSM[pillar_idx_inBlock][point_idx][k] =
            voxel_features[3 * (dense_pillar_idx * max_points + point_idx) + k];
    }
    __syncthreads();
    // calculate sm
    if (point_idx < pointsNumSM[pillar_idx_inBlock]) {
    ...
    if (point_idx < pointsNumSM[pillar_idx_inBlock]) {
        for (int k = 0; k < 3; k++)
            pillarOutSM[pillar_idx_inBlock][point_idx][k] = pillarSM[pillar_idx_inBlock][point_idx][k];
        pillarOutSM[pillar_idx_inBlock][point_idx][3] = mean.x;
        pillarOutSM[pillar_idx_inBlock][point_idx][3 + 1] = mean.y;
        pillarOutSM[pillar_idx_inBlock][point_idx][3 + 2] = mean.z;
        pillarOutSM[pillar_idx_inBlock][point_idx][3 + 3] = center.x;
        pillarOutSM[pillar_idx_inBlock][point_idx][3 + 4] = center.y;
        if (3 + 5 < voxel_features_size) pillarOutSM[pillar_idx_inBlock][point_idx][warp_size + 5] = center.z;
    } else {
        for (int k = 0; k < voxel_features_size; k++) pillarOutSM[pillar_idx_inBlock][point_idx][k] = 0;
    }
    __syncthreads();
...
__global__ void generateFeatures_kernel_4x(int batch_size, int dense_pillar_num, float *voxel_features,
                                           unsigned int *voxel_num_points, unsigned int *coords, unsigned int *params,
                                           float voxel_x, float voxel_y, float voxel_z, float range_min_x,
                                           float range_min_y, float range_min_z, unsigned int voxel_features_size,
                                           unsigned int max_points, unsigned int max_voxels, float *features) {
...
int generateFeatures_launch(int batch_size, int dense_pillar_num, float *voxel_features, unsigned int *voxel_num_points,
    ...
    if (num_point_values == 4) {
        generateFeatures_kernel_4x<<<blocks, threads, 0, stream>>>(
            batch_size, dense_pillar_num, voxel_features, voxel_num_points, coords, params, voxel_x, voxel_y, voxel_z,
            range_min_x, range_min_y, range_min_z, voxel_features_size, max_points, max_voxels, features);
    } else if (num_point_values == 3) {
        generateFeatures_kernel_3x<<<blocks, threads, 0, stream>>>(
            batch_size, dense_pillar_num, voxel_features, voxel_num_points, coords, params, voxel_x, voxel_y, voxel_z,
            range_min_x, range_min_y, range_min_z, voxel_features_size, max_points, max_voxels, features);
    } else {
