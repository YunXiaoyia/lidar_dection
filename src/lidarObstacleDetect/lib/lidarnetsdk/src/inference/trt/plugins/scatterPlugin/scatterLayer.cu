...
__global__ void scatterKernel(char* output, const char* updates, const int* indices, int pitch, int rowSize) {
...
__global__ void transformIdxKernel(
    ...
    const int* indices, int sliceRank) {
...
pluginStatus_t scatterNDInference(cudaStream_t stream, int* transformCoeff, int nOutputDims, int sliceRank, int nRows,
