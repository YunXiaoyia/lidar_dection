...
DimsExprs ScatterND::getOutputDimensions(int32_t outputIndex, const DimsExprs* inputs, int32_t nbInputs,
...
int ScatterND::initialize() noexcept { return 0; }

void ScatterND::terminate() noexcept {}

bool ScatterND::supportsFormatCombination(int32_t pos, const PluginTensorDesc* inOut, int32_t nbInputs,
...
void ScatterND::configurePlugin(const DynamicPluginTensorDesc* in, int32_t nbInputs, const DynamicPluginTensorDesc* out,
                                int32_t nbOutputs) noexcept {}
...
size_t ScatterND::getWorkspaceSize(const PluginTensorDesc* inputs, int32_t nbInputs, const PluginTensorDesc* outputs,
...
void ScatterND::calculateTransformCoeff(const Dims& dataTensorDims, int indexRank,
                                        int32_t* transformCoeff) const noexcept {
...
int32_t ScatterND::enqueue(const PluginTensorDesc* inputDesc, const PluginTensorDesc* outputDesc,
    ...
    scatterNDInference(stream, transformCoeff, dataDims.nbDims, indexRank, nSlices, rowSize, copySize,
                       elementSizeInBytes, inputs[indexTensorIdx], inputs[updateTensorIdx], inputs[dataTensorIdx],
                       outputs[0], workspace);
...
void ScatterND::serialize(void* buffer) const noexcept { return; }

// Set plugin namespace
void ScatterND::setPluginNamespace(const char* pluginNamespace) noexcept { mPluginNamespace = pluginNamespace; }

const char* ScatterND::getPluginNamespace() const noexcept { return mPluginNamespace.c_str(); }
...
const char* ScatterND::getPluginType() const noexcept { return SCATTERND_PLUGIN_NAME; }

const char* ScatterND::getPluginVersion() const noexcept { return SCATTERND_PLUGIN_VERSION; }

void ScatterND::destroy() noexcept { delete this; }
...
ScatterNDPluginCreator::ScatterNDPluginCreator() { mFC.nbFields = 0; }

const char* ScatterNDPluginCreator::getPluginName() const noexcept { return SCATTERND_PLUGIN_NAME; }

const char* ScatterNDPluginCreator::getPluginVersion() const noexcept { return SCATTERND_PLUGIN_VERSION; }

const PluginFieldCollection* ScatterNDPluginCreator::getFieldNames() noexcept { return &mFC; }
...
IPluginV2Ext* ScatterNDPluginCreator::deserializePlugin(const char* name, const void* serialData,
