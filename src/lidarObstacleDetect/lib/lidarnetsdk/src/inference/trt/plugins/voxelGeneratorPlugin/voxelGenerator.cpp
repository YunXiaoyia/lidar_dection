...
#include "voxelGeneratorKernel.cuh"
...
VoxelGeneratorPlugin::VoxelGeneratorPlugin(int max_voxels, int max_points, int voxel_features, float x_min, float x_max,
                                           float y_min, float y_max, float z_min, float z_max, float pillar_x,
                                           float pillar_y, float pillar_z)
    : pillarNum_(max_voxels),
      pointNum_(max_points),
      featureNum_(voxel_features),
      min_x_range_(x_min),
      max_x_range_(x_max),
      min_y_range_(y_min),
      max_y_range_(y_max),
      min_z_range_(z_min),
      max_z_range_(z_max),
      pillar_x_size_(pillar_x),
      pillar_y_size_(pillar_y),
...
VoxelGeneratorPlugin::VoxelGeneratorPlugin(int max_voxels, int max_points, int voxel_features, float x_min, float x_max,
                                           float y_min, float y_max, float z_min, float z_max, float pillar_x,
                                           float pillar_y, float pillar_z, int point_features, int grid_x, int grid_y,
                                           int grid_z)
    : pillarNum_(max_voxels),
      pointNum_(max_points),
      featureNum_(voxel_features),
      ...
      pillar_y_size_(pillar_y),
      pillar_z_size_(pillar_z),
      pointFeatureNum_(point_features),
      grid_x_size_(grid_x),
      grid_y_size_(grid_y),
      grid_z_size_(grid_z) {}
...
nvinfer1::IPluginV2DynamicExt* VoxelGeneratorPlugin::clone() const noexcept {
    auto* plugin =
        new VoxelGeneratorPlugin(pillarNum_, pointNum_, featureNum_, min_x_range_, max_x_range_, min_y_range_,
                                 max_y_range_, min_z_range_, max_z_range_, pillar_x_size_, pillar_y_size_,
                                 pillar_z_size_, pointFeatureNum_, grid_x_size_, grid_y_size_, grid_z_size_);
    plugin->setPluginNamespace(mNamespace.c_str());
    return plugin;
}

nvinfer1::DimsExprs VoxelGeneratorPlugin::getOutputDimensions(int outputIndex, const nvinfer1::DimsExprs* inputs,
                                                              int nbInputs,
...
bool VoxelGeneratorPlugin::supportsFormatCombination(int pos, const nvinfer1::PluginTensorDesc* inOut, int nbInputs,
...
size_t VoxelGeneratorPlugin::getWorkspaceSize(const nvinfer1::PluginTensorDesc* inputs, int nbInputs,
    ...
    size_t voxels_size =
        batchSize * grid_z_size_ * grid_y_size_ * grid_x_size_ * pointNum_ * pointFeatureNum_ * sizeof(float);
...
int VoxelGeneratorPlugin::enqueue(const nvinfer1::PluginTensorDesc* inputDesc,
    ...
    size_t voxels_size = batchSize * dense_pillar_num * pointNum_ * pointFeatureNum_ * sizeof(float);
    size_t voxel_features_size = voxels_size;
    size_t voxel_num_points_size = mask_size;
    size_t workspaces[4];
    workspaces[0] = mask_size;
    ...
    float* voxels_ = reinterpret_cast<float*>(nextWorkspacePtr(reinterpret_cast<int8_t*>(mask_), mask_size));
    float* voxel_features_ =
        reinterpret_cast<float*>(nextWorkspacePtr(reinterpret_cast<int8_t*>(voxels_), voxels_size));
    ...
    generateVoxels_launch(batchSize, maxNumPoints, pointCloud, pointNum, min_x_range_, max_x_range_, min_y_range_,
                          max_y_range_, min_z_range_, max_z_range_, pillar_x_size_, pillar_y_size_, pillar_z_size_,
                          grid_y_size_, grid_x_size_, pointFeatureNum_, pointNum_, mask_, voxels_, stream);
    // mask_ + voxel_ ---> params_data + voxel_features_ + voxel_num_points_ + coords_data
    generateBaseFeatures_launch(batchSize, mask_, voxels_, grid_y_size_, grid_x_size_, params_data, pillarNum_,
                                pointNum_, pointFeatureNum_, voxel_features_, voxel_num_points_, coords_data, stream);
    generateFeatures_launch(batchSize, dense_pillar_num, voxel_features_, voxel_num_points_, coords_data, params_data,
                            pillar_x_size_, pillar_y_size_, pillar_z_size_, min_x_range_, min_y_range_, min_z_range_,
                            featureNum_, pointNum_, pillarNum_, pointFeatureNum_, pillar_features_data, stream);
    return 0;
}

nvinfer1::DataType VoxelGeneratorPlugin::getOutputDataType(int index, const nvinfer1::DataType* inputTypes,
                                                           int nbInputs) const noexcept {
    if (index == 0) return inputTypes[0];
    return inputTypes[1];
}

const char* VoxelGeneratorPlugin::getPluginType() const noexcept { return PLUGIN_NAME; }

const char* VoxelGeneratorPlugin::getPluginVersion() const noexcept { return PLUGIN_VERSION; }

int VoxelGeneratorPlugin::getNbOutputs() const noexcept { return 3; }

int VoxelGeneratorPlugin::initialize() noexcept { return 0; }

void VoxelGeneratorPlugin::terminate() noexcept {}

size_t VoxelGeneratorPlugin::getSerializationSize() const noexcept { return 9 * sizeof(float) + 7 * sizeof(int); }
...
void VoxelGeneratorPlugin::destroy() noexcept { delete this; }

void VoxelGeneratorPlugin::setPluginNamespace(const char* libNamespace) noexcept { mNamespace = libNamespace; }

const char* VoxelGeneratorPlugin::getPluginNamespace() const noexcept { return mNamespace.c_str(); }
...
const char* VoxelGeneratorPluginCreator::getPluginName() const noexcept { return PLUGIN_NAME; }

const char* VoxelGeneratorPluginCreator::getPluginVersion() const noexcept { return PLUGIN_VERSION; }

const PluginFieldCollection* VoxelGeneratorPluginCreator::getFieldNames() noexcept { return &mFC; }
...
IPluginV2* VoxelGeneratorPluginCreator::createPlugin(const char* name, const PluginFieldCollection* fc) noexcept {
    ...
    IPluginV2* plugin = new VoxelGeneratorPlugin(
        max_voxels, max_points, voxel_feature_num, point_cloud_range[0], point_cloud_range[3], point_cloud_range[1],
        point_cloud_range[4], point_cloud_range[2], point_cloud_range[5], voxel_size[0], voxel_size[1], voxel_size[2]);
    return plugin;
}

IPluginV2* VoxelGeneratorPluginCreator::deserializePlugin(const char* name, const void* serialData,
                                                          size_t serialLength) noexcept {
    return new VoxelGeneratorPlugin(serialData, serialLength);
}

void VoxelGeneratorPluginCreator::setPluginNamespace(const char* libNamespace) noexcept { mNamespace = libNamespace; }

const char* VoxelGeneratorPluginCreator::getPluginNamespace() const noexcept { return mNamespace.c_str(); }
