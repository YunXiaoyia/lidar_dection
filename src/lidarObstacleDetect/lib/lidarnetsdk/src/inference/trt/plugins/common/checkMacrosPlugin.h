...
[[noreturn]] void throwCublasError(const char* file, const char* function, int line, int status,
                                   const char* msg = nullptr);
[[noreturn]] void throwPluginError(char const* file, char const* function, int line, int status,
...
inline void caughtError(const std::exception& e) { gLogError << e.what() << std::endl; }
}  // namespace plugin
