    ...
    void addPluginCreator(void* logger, const char* libNamespace) {
        ...
        if (mRegistryList.find(pluginType) == mRegistryList.end()) {
            ...
            if (status) {
                ...
                throw std::runtime_error("Could not register plugin creator -  " + pluginType);
            }
        } else {
...
bool initLibNvInferPlugins(void* logger, const char* libNamespace) {
    ...
    initializePlugin<nvinfer1::plugin::PillarScatterCamPluginCreator>(logger, libNamespace);
    ...
    initializePlugin<nvinfer1::plugin::MultiViewVoxelGeneratorPluginCreator>(logger, libNamespace);

    /* MMDeploy */
    initializePlugin<mmdeploy::TRTGridSamplerCreator>(logger, libNamespace);
