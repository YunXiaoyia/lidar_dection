...
struct FlatFormerParamV2 {
    FlatFormerParamV2() = default;
    ...
    bool Init(const nlohmann::json& param_node) noexcept {
        try {
            ...
            GET_VALUE(param_node, min_group_num);
            GET_VALUE(param_node, min_voxel_num);

            return true;
        } catch (const std::exception& e) {
            PLOG_FATAL << "[Detector] FlatFormer Param JSON config failed with: " << e.what();
        }
        return false;
    }
