...
VisualizeDetectedObjects::ObjectsToHulls(const autoware_msgs::DetectedObjectArray &in_objects)
{
  ...
  for (auto const &object: in_objects.objects)
  {
    if (IsObjectValid(object) && !object.convex_hull.polygon.points.empty() && object.label == "unknown")
    {
      ...
      hull.type = visualization_msgs::Marker::LINE_LIST;
      ...
      size_t num_points = object.convex_hull.polygon.points.size();
      double height = object.dimensions.z;

      for(size_t i = 0; i < num_points; ++i)
      {
          // 获取当前点和下一点（构成底边）
          const auto& p32_1 = object.convex_hull.polygon.points[i];
          const auto& p32_2 = object.convex_hull.polygon.points[(i + 1) % num_points];

          geometry_msgs::Point p_base_1;
          p_base_1.x = p32_1.x;
          p_base_1.y = p32_1.y;
          p_base_1.z = p32_1.z;

          geometry_msgs::Point p_base_2;
          p_base_2.x = p32_2.x;
          p_base_2.y = p32_2.y;
          p_base_2.z = p32_2.z;

          // 计算对应的顶边点
          geometry_msgs::Point p_top_1 = p_base_1;
          p_top_1.z += height;
          geometry_msgs::Point p_top_2 = p_base_2;
          p_top_2.z += height;

          // 1. 画底边
          hull.points.push_back(p_base_1);
          hull.points.push_back(p_base_2);

          // 2. 画顶边
          hull.points.push_back(p_top_1);
          hull.points.push_back(p_top_2);
