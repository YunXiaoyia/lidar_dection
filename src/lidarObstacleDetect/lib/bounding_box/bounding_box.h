...
public:
    ...
    // 计算多边形（凹凸包）
    void ComputePolygon(std_msgs::Header header);
    ...
    double boxMergeThreshold_;

    double concave_hull_dist_;
    double concave_hull_alpha_;

    // 大物体判断阈值（新增）
    float large_object_length_threshold_;  // 长度阈值
    float large_object_width_threshold_;   // 宽度阈值
};
